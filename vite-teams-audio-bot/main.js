...
export async function joinAndPlay() {
  ...
  activeCall.on('remoteParticipantsUpdated', (e) => {
    ...
    e.added.forEach(participant => {
      console.log(`Remote participant joined: ${participant.displayName || 'Unknown'}`);
      setupRemoteParticipant(participant);
    });
    
    // Handle participants leaving
    e.removed.forEach(participant => {
      console.log(`Remote participant left: ${participant.identifier.communicationUserId}`);
      remoteParticipantsAudio.delete(participant.identifier.communicationUserId);
    });
  });
  
  // Set up existing remote participants
  activeCall.remoteParticipants.forEach(participant => {
    console.log(`Setting up existing remote participant: ${participant.displayName || 'Unknown'}`);
    setupRemoteParticipant(participant);
  });
}
...
function setupRemoteParticipant(participant) {
  ...
  participant.on('streamAdded', (e) => {
    if (e.stream.mediaStreamType === 'Audio') {
      console.log(`Remote participant ${participant.displayName || 'Unknown'} added audio stream`);
      ...
      if (isCapturingAudio) {
        captureParticipantAudio(participant.identifier.communicationUserId, e.stream);
      }
    }
  });
  ...
  participant.on('streamRemoved', (e) => {
    if (e.stream.mediaStreamType === 'Audio') {
      console.log(`Remote participant ${participant.displayName || 'Unknown'} removed audio stream`);
      remoteParticipantsAudio.delete(participant.identifier.communicationUserId);
    }
  });
}
...
function captureParticipantAudio(participantId, stream) {
  ...
  try {
    ...
    console.log(`Successfully capturing audio from participant ${participantId}`);
    ...
    transcriptionArea.value += `[${timestamp}] Started capturing audio from participant ${participantId}\n`;
    ...
    speechRecognizer.recognized = (s, e) => {
      if (e.result.reason === SpeechSDK.ResultReason.RecognizedSpeech) {
        ...
        if (transcript && transcript.trim() !== "") {
          ...
          // Add the transcript to the transcription area
          const transcriptionArea = document.getElementById("transcriptionArea");
          const timestamp = new Date().toLocaleTimeString();
          transcriptionArea.value += `[${timestamp}] ${transcript}\n`;
          transcriptionArea.scrollTop = transcriptionArea.scrollHeight;
        }
      }
    };
