import { CallClient, LocalAudioStream, Features } from "@azure/communication-calling";
...
export async function joinAndPlay() {
  ...
  try {
    const dominantSpeakersFeature = activeCall.feature(Features.DominantSpeakers);
    
    // Subscribe to dominant speakers changes
    dominantSpeakersFeature.on('dominantSpeakersChanged', () => {
      const dominantSpeakers = dominantSpeakersFeature.dominantSpeakers.speakersList;
      if (dominantSpeakers.length > 0) {
        ...
        const dominantParticipant = activeCall.remoteParticipants.find(
          p => p.identifier.communicationUserId === dominantSpeakerId.communicationUserId
        );
        
        if (dominantParticipant) {
          const participantId = dominantParticipant.identifier.communicationUserId;
          ...
          for (const [id, data] of remoteParticipantsAudio.entries()) {
            ...
            if (id.startsWith('stream-')) {
              console.log(`Associating stream ${id} with participant ${speakerName}`);
              ...
              if (dominantParticipant.isSpeaking && !speechRecognizers.has(participantId)) {
...
function setupRemoteParticipant(participant) {
  console.log("Setting up remote participant:", participant.displayName || "Unknown");
    ...
    if (participant.isSpeaking) {
      ...
      if (isCapturingAudio) {
        ...
        if (!speechRecognizers.has(participantId)) {
          ...
          if (audioStream) {
            console.log(`Setting up speech recognition for active speaker: ${participantName}`);
  ...
  participant.on("streamAdded", (e) => {
    ...
    if (e.stream.mediaStreamType === "Audio") {
      ...
      console.log(
        `Remote participant ${participantName} added audio stream`
      );
...
window.testSpeechRecognition = async function() {
  ...
  try {
    ...
    const transcriptionArea = document.getElementById("transcriptionArea");
    transcriptionArea.value += "Speech recognition test started. Speak into your microphone.\n";
    transcriptionArea.value += "Call window.stopTestSpeechRecognition() to stop the test.\n";
    transcriptionArea.scrollTop = transcriptionArea.scrollHeight;
  } catch (error) {
...
window.startCapturingAudio = async function () {
  ...
  console.log(`Found ${activeCall.remoteAudioStreams.length} remote audio streams`);
  ...
  activeCall.on("remoteAudioStreamsUpdated", (e) => {
    console.log(`Remote audio streams updated: ${e.added.length} added, ${e.removed.length} removed`);
    ...
    e.removed.forEach(audioStream => {
      ...
      for (const [id, data] of remoteParticipantsAudio.entries()) {
        if (data.stream === audioStream) {
          console.log(`Removing audio stream for participant ${data.participantName || id}`);
...
function captureParticipantAudio(participantId, stream, participant = null) {
  ...
  try {
    ...
    if (participant) {
      participantName = participant.displayName || participantId.substring(0, 8);
    } else if (activeCall) {
      const foundParticipant = activeCall.remoteParticipants.find(
        (p) => p.identifier.communicationUserId === participantId
      );
      if (foundParticipant) {
        participantName = foundParticipant.displayName || participantId.substring(0, 8);
      }
    }
    ...
    if (stream && typeof stream.getMediaStream === 'function') {
      ...
      stream.getMediaStream().then(mediaStream => {
    ...
    console.log(`Successfully capturing audio from participant ${participantName}`);
    ...
    console.error(`Error capturing audio from participant ${participantId}:`, error);
  }
}
...
function setupSpeechRecognizerForParticipant(participantId, participantName) {
  try {
    console.log(`Setting up speech recognizer for participant ${participantName}`);
    
    // Get the audio element for this participant
    const data = remoteParticipantsAudio.get(participantId);
    if (!data || !data.audioElement) {
      console.error(`No audio element found for participant ${participantName}`);
      return;
    }
    ...
    try {
      ...
      if (audioElement.srcObject && audioElement.srcObject instanceof MediaStream) {
        ...
        audioConfig = SpeechSDK.AudioConfig.fromStreamInput(audioElement.srcObject);
      } else {
    ...
    participantRecognizer.startContinuousRecognitionAsync(
      () => {
        console.log(`Speech recognition started for participant ${participantName}`);
      },
      (err) => {
        console.error(`Error starting speech recognition for participant ${participantName}:`, err);
      }
    );
    ...
    participantRecognizer.recognizing = (s, e) => {
      ...
      if (e.result.reason === SpeechSDK.ResultReason.RecognizingSpeech) {
        const transcript = e.result.text;
        if (transcript && transcript.trim() !== "") {
          console.log(`Recognizing speech from ${participantName}: ${transcript}`);
        }
      }
    };
    ...
    console.error(`Error setting up speech recognition for participant ${participantName}:`, error);
  }
}
